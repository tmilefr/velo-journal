...
- **🖼️ Affiche** (`/affiche`) — la carte du voyage au format **A3**, à imprimer et encadrer : les traces GPX sur un fond épuré (littoraux, frontières, villes, relief ombré) et la photo favorite de **chaque étape** disposée tout autour du cadre, chacune reliée par un fil à son point d'arrivée. Les vignettes s'adaptent au carnet — grandes photos pour une dizaine d'étapes, anneaux de miniatures au-delà — et la carte occupe environ un tiers de la feuille, à la forme du voyage. Portrait ou paysage, export PNG 150 / 300 dpi, impression A3 directe. Le relief est échantillonné via [Open-Meteo](https://open-meteo.com/) puis mis en cache dans `tmp/relief/` ; sans réseau, l'affiche se dessine simplement sans ombrage
