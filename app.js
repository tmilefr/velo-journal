...
app.get('/', requireFamily, (req, res) => {
  ...
  req.session.save(() => res.send(renderPublic(posts, !!req.session.auth || !!req.session.margot, token)));
});
...
app.get('/preparation', requireFamily, (req, res) => {
  ...
  const token = csrfToken(req);
  req.session.save(() => res.send(renderPreparation(posts, !!req.session.auth || !!req.session.margot, token)));
});
...
app.post('/post', requireAuth, requireCsrf, upload.fields([{name:'photos', maxCount:10},{name:'gpx', maxCount:1}]), async (req, res) => {
  ...
  if (postDate) { const parsed = new Date(postDate); if (!isNaN(parsed.getTime())) finalDate = parsed.toISOString(); }

  let finalLat = parseFloat(lat) || null;
  let finalLon = parseFloat(lon) || null;
  if (gpxFile) {
    try {
      const gpxText = fs.readFileSync(path.join(__dirname, 'public', gpxFile), 'utf8');
      const matches = [...gpxText.matchAll(/<trkpt\s+lat="([^"]+)"\s+lon="([^"]+)"/g)];
      if (matches.length > 0) { const last = matches[matches.length - 1]; finalLat = parseFloat(last[1]); finalLon = parseFloat(last[2]); }
    } catch(e) {}
  }
...
function renderHeader({ activePage = '', isAdmin = false, showMap = false } = {}) {
  const links = [
    ...
    ...(showMap ? [{ href: '/map', label: 'Carte', key: 'map', icon: '🗺️' }] : []),
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .card-text{font-size:14.5px;color:var(--ink-mid);line-height:1.75;white-space:pre-wrap;}
  ...
  .timeline-thumb{width:80px;height:70px;object-fit:cover;border-radius:8px;flex-shrink:0;}
...
function previewPhotos(input) {
  ...
  Array.from(input.files).forEach(function(f){
    var img=document.createElement('img'); img.src=URL.createObjectURL(f); preview.appendChild(img);
  });
}
</script>
`;
...
<script>
  (function(){
    var lb=document.getElementById('lb'),lbImg=document.getElementById('lb-img'),lbCounter=document.getElementById('lb-counter'),postImgs=[],cur=0;
...
function renderCard(p, isAdmin, csrf) {
...
function renderPublic(posts, isAdmin = false, csrf = '') {
  ...
  const postCards = posts.length === 0
    ...
    : posts.map(p => renderCard(p, isAdmin, csrf)).join('');
...
function renderMap(posts, isAdmin = false) {
  ...
  const gpsJson = JSON.stringify(withGps.map(p => ({
    ...
    photo: p.photos?.[0] || null, gpx: p.gpx || null,
  })));
...
function renderPostForm(err, lastLocation = '', isMargot = false, csrf = '', defaultType = '') {
  return `<!DOCTYPE html><html lang="fr"><head>
    ...
    <div class="form-wrap"><div class="form-card">
      ...
      <form method="POST" action="/post?_csrf=${csrf}" enctype="multipart/form-data">
        <input type="hidden" name="_csrf" value="${csrf}">
        <div class="field">
